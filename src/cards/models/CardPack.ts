...
export default class CardPack {
  ...
  createDefaultPackage(): Card[] {
    ...
    for (let i = 0; i < this.DEFAULT_PACKAGE_COMMON; i++) {
      defaultPackage.push(
        this.selectRandomCardFromCards(cardsByRarity[RARITY_COMMON])
      );
    }

    for (let i = 0; i < this.DEFAULT_PACKAGE_UNCOMMON; i++) {
      defaultPackage.push(
        this.selectRandomCardFromCards(cardsByRarity[RARITY_UNCOMMON])
      );
    }
    ...
    for (let i = 0; i < this.DEFAULT_PACKAGE_RARE; i++) {
      if (this.isOneOfProbability(3)) {
        defaultPackage.push(
          this.selectRandomCardFromCards(cardsByRarity[RARITY_RARE])
        );
      } else {
        defaultPackage.push(
          this.selectRandomCardFromCards(cardsByRarity[RARITY_UNCOMMON])
