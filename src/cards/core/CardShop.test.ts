...
test("test not sufficient pokecoins of user error", async () => {
  ...
  expect(cardShop.buyDefaultPackage("Base")).rejects.toStrictEqual(
    new NotSufficientCoinsError(0, cardShop.DEFAULT_PACKAGE_COST)
  );
});
...
test("test card pack not found error", async () => {
  const user = new UserModel({ username: "test", password: "test" });
  const cardShop = new CardShop(user);
  expect(cardShop.buyDefaultPackage("NotFound")).rejects.toStrictEqual(
    new CardPackNotFoundError("NotFound")
  );
});
...
test("test buyDefaultPackage", async () => {
  ...
  cardShop.wallet.withdraw = jest.fn(async (amount) => {
